import { NOT_STITCHABLE_NUM, type ShapeGrid } from "./shape-border";
...
export function tracePerimeter(
  ...
  outer: for (let y = 0; y < grid.height; y++) {
    for (let x = 0; x < grid.width; x++) {
      if (stitchable(grid, x, y, sentinel)) {
        start = { x, y };
        break outer;
      }
    }
  ...
  do {
    ...
    !(
      `${current.x},${current.y}` === firstStep.p &&
      `${backtrack.x},${backtrack.y}` === firstStep.b
    )
  );
...
export function detectCorners(
  ...
  if (
    merged.length > 1 &&
    merged[0] + points.length - merged[merged.length - 1] <= window
  ) {
    merged.pop();
  }
  return merged;
...
export function distributeRepeats(
  ...
  if (count < 1) {
    return { placements: [], count: 0, remainder: total, tooShort: true };
  }
